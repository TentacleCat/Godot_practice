...
func _ready() -> void:
	...
	for column in range(column_count):
		...
		for row in range(row_count):
			...
			cell_martrix[column].push_back(cell)
...
func _process(delta: float) -> void:
	for column in range(column_count):
		for row in range(row_count):
			pre_cell_states[column][row] = cell_martrix[column][row].visible
			if !is_edge(column, row):
				cell_martrix[column][row].visible = get_next_state(column, row)
	pass
